...
module @gemm attributes {gpu.container_module} {
  func.func @test(%A: memref<?x?xf16>, %B: memref<?x?xf16>, %C: memref<?x?xf32>, %M: index, %K: index, %N: index,  %wg_M: index, %wg_K: index, %wg_N: index, %sg_M: index, %sg_K: index, %sg_N: index, %wg_X: index, %wg_Y: index, %wg_Z: index, %sg_X: index, %sg_Y: index, %sg_Z: index, %prefetch_distance: index, %barrier_distance: index) -> memref<?x?xf32> attributes {llvm.emit_c_interface} {

    // %A_gpu = gpu.alloc  host_shared (%M, %K) : memref<?x?xf16>
    // memref.copy %A, %A_gpu : memref<?x?xf16> to memref<?x?xf16>
    // %B_gpu = gpu.alloc  host_shared (%K, %N) : memref<?x?xf16>
    // memref.copy %B, %B_gpu : memref<?x?xf16> to memref<?x?xf16>
    // %C_gpu = gpu.alloc  host_shared (%M, %N) : memref<?x?xf32>
    // memref.copy %C, %C_gpu : memref<?x?xf32> to memref<?x?xf32>
    ...
    gpu.memcpy %C_gpu, %C : memref<?x?xf32>, memref<?x?xf32>


    gpu.launch_func  @gemm_kernels::@gemm_kernel_wgX_256_wgY_256_Kblock_32_sgX_32_sgY_64_split_barrier blocks in (%wg_X, %wg_Y, %wg_Z) threads in (%sg_X, %sg_Y, %sg_Z) args(%A_gpu : memref<?x?xf16>, %B_gpu : memref<?x?xf16>, %C_gpu : memref<?x?xf32>, %M: index, %K: index, %N: index,%wg_M: index, %wg_K: index, %wg_N: index, %sg_M: index, %sg_K: index, %sg_N: index, %sg_X: index, %sg_Y: index, %prefetch_distance : index, %barrier_distance : index)

    // gpu.launch_func  @gemm_kernels::@gemm_kernel_wgX_256_wgY_256_Kblock_32_sgX_32_sgY_64_named_barrier blocks in (%wg_X, %wg_Y, %wg_Z) threads in (%sg_X, %sg_Y, %sg_Z) args(%A_gpu : memref<?x?xf16>, %B_gpu : memref<?x?xf16>, %C_gpu : memref<?x?xf32>, %M: index, %K: index, %N: index,%wg_M: index, %wg_K: index, %wg_N: index, %sg_M: index, %sg_K: index, %sg_N: index, %sg_X: index, %sg_Y: index, %prefetch_distance : index, %barrier_distance : index)

    %C_host = memref.alloc (%M, %N) : memref<?x?xf32>
    gpu.memcpy %C_host, %C_gpu : memref<?x?xf32>, memref<?x?xf32>
    gpu.dealloc  %A_gpu : memref<?x?xf16>
    gpu.dealloc  %B_gpu : memref<?x?xf16>
    gpu.dealloc  %C_gpu : memref<?x?xf32>
    return %C_host : memref<?x?xf32>

    // gpu.dealloc  %A_gpu : memref<?x?xf16>
    // gpu.dealloc  %B_gpu : memref<?x?xf16>
    // return %C_gpu : memref<?x?xf32>
  }
  gpu.module @gemm_kernels attributes {spirv.target_env = #spirv.target_env<#spirv.vce<v1.4, [Addresses, Float16Buffer, Int64, Int16, Int8, Kernel, Linkage, Vector16, GenericPointer, Groups, Float16, Float64, AtomicFloat32AddEXT, ExpectAssumeKHR, SubgroupDispatch, VectorComputeINTEL, VectorAnyINTEL], [SPV_EXT_shader_atomic_float_add, SPV_KHR_expect_assume, SPV_INTEL_vector_compute]>, api=OpenCL, #spirv.resource_limits<>>} {
    ...
    gpu.func @gemm_kernel_wgX_256_wgY_256_Kblock_32_sgX_32_sgY_64_split_barrier(%A: memref<?x?xf16>, %B: memref<?x?xf16>, %C: memref<?x?xf32>, %M: index, %K: index, %N: index,  %wg_M: index, %wg_K: index, %wg_N: index, %sg_M: index, %sg_K: index, %sg_N: index, %sg_X: index, %sg_Y: index, %prefetch_distance : index, %barrier_distance : index) kernel attributes {VectorComputeFunctionINTEL, spirv.entry_point_abi = #spirv.entry_point_abi<>} {
      // constants
      ...
      %wg_tile_offset_x = arith.muli %wg_id_x, %wg_M : index
      %wg_tile_offset_y = arith.muli %wg_id_y, %wg_N : index
      ...
      %A_nth_prefetch_tile = scf.for %i = %c0 to %prefetch_distance step %c1 iter_args(%current_tile = %A_sg_prefetch_tile_iter0) -> !xegpu.tensor_desc<8x32xf16> {
        ...
        %next_tile = xegpu.update_nd_offset %current_tile, [%c0, %c32] : !xegpu.tensor_desc<8x32xf16>
        scf.yield %next_tile : !xegpu.tensor_desc<8x32xf16>
      }

      // %is_prefetch_distance_zero = arith.cmpi eq, %prefetch_distance, %c0 : index

      // If prefetch distance is zero, we use the first tile, otherwise we use the last prefetched tile
      // %A_nth_prefetch_tile = arith.select %is_prefetch_distance_zero, %A_sg_prefetch_tile_iter0, %A_nth_prefetch_tile_tmp : !xegpu.tensor_desc<8x32xf16>

      // %A_nth_prefetch_tile = scf.if %is_prefetch_distance_zero -> !xegpu.tensor_desc<8x32xf16> {
      //   // If prefetch distance is zero, we use the first tile
      //   scf.yield %A_sg_prefetch_tile_iter0 : !xegpu.tensor_desc<8x32xf16>
      // } else {
      //   // Otherwise we use the last prefetched tile
      //   scf.yield %A_nth_prefetch_tile_tmp : !xegpu.tensor_desc<8x32xf16>
      // }
      ...
      %B_nth_prefetch_tile = scf.for %i = %c0 to %prefetch_distance step %c1 iter_args(%current_tile = %B_sg_prefetch_tile_iter0) -> !xegpu.tensor_desc<8x32xf16> {
        xegpu.prefetch_nd %current_tile {l1_hint = #xegpu.cache_hint<cached>, l2_hint = #xegpu.cache_hint<cached>, l3_hint = #xegpu.cache_hint<cached>} : !xegpu.tensor_desc<8x32xf16>
        %next_tile = xegpu.update_nd_offset %current_tile, [%c32, %c0] : !xegpu.tensor_desc<8x32xf16>
        scf.yield %next_tile : !xegpu.tensor_desc<8x32xf16>
      }

      // %B_nth_prefetch_tile = arith.select %is_prefetch_distance_zero, %B_sg_prefetch_tile_iter0, %B_nth_prefetch_tile_tmp : !xegpu.tensor_desc<8x32xf16>
      // %B_nth_prefetch_tile = scf.if %is_prefetch_distance_zero -> !xegpu.tensor_desc<8x32xf16> {
      //   // If prefetch distance is zero, we use the first tile
      //   scf.yield %B_sg_prefetch_tile_iter0 : !xegpu.tensor_desc<8x32xf16>
      // } else {
      //   // Otherwise we use the last prefetched tile
      //   scf.yield %B_nth_prefetch_tile_tmp : !xegpu.tensor_desc<8x32xf16>
      // }

      %A_sg_init_tile_0 = xegpu.create_nd_tdesc %A[%C_sg_tile_offset_x, %c0], [%M, %K], [%K, %c1] : memref<?x?xf16> -> !xegpu.tensor_desc<16x16xf16, #xegpu.block_tdesc_attr<array_length =2>>
      %A_sg_init_tile_1 = xegpu.update_nd_offset %A_sg_init_tile_0, [%c16, %c0] : !xegpu.tensor_desc<16x16xf16, #xegpu.block_tdesc_attr<array_length =2>>

      //create B tiles
      %B_sg_init_tile_0 = xegpu.create_nd_tdesc %B[%c0, %C_sg_tile_offset_y], [%K, %N], [%N, %c1] : memref<?x?xf16> -> !xegpu.tensor_desc<16x16xf16, #xegpu.block_tdesc_attr<array_length =2>>
      %B_sg_init_tile_1 = xegpu.update_nd_offset %B_sg_init_tile_0, [%c0, %c32] :  !xegpu.tensor_desc<16x16xf16, #xegpu.block_tdesc_attr<array_length =2>>

      %B_sg_init_tile_2 = xegpu.update_nd_offset %B_sg_init_tile_0, [%c16, %c0] :  !xegpu.tensor_desc<16x16xf16, #xegpu.block_tdesc_attr<array_length =2>>
      %B_sg_init_tile_3 = xegpu.update_nd_offset %B_sg_init_tile_2, [%c0, %c32] :  !xegpu.tensor_desc<16x16xf16, #xegpu.block_tdesc_attr<array_length =2>>
      // ************************* //
      ...
      %k_loop_result:24 = scf.for %k = %c0 to %K step %c32 iter_args (
        ...
        %every_nth_iter = arith.remui %k, %c256 : index
        ...
        // load B tiles
        %b_val_0 = xegpu.load_nd %B_tile_0 {packed, l1_hint = #xegpu.cache_hint<cached>, l2_hint = #xegpu.cache_hint<cached>, l3_hint = #xegpu.cache_hint<cached>} : !xegpu.tensor_desc<16x16xf16, #xegpu.block_tdesc_attr<array_length =2>> -> vector<2x8x16x2xf16>
        %b_val_1 = xegpu.load_nd %B_tile_1 {packed, l1_hint = #xegpu.cache_hint<cached>, l2_hint = #xegpu.cache_hint<cached>, l3_hint = #xegpu.cache_hint<cached>} : !xegpu.tensor_desc<16x16xf16, #xegpu.block_tdesc_attr<array_length =2>> -> vector<2x8x16x2xf16>
        %b_val_2 = xegpu.load_nd %B_tile_2 {packed, l1_hint = #xegpu.cache_hint<cached>, l2_hint = #xegpu.cache_hint<cached>, l3_hint = #xegpu.cache_hint<cached>} : !xegpu.tensor_desc<16x16xf16, #xegpu.block_tdesc_attr<array_length =2>> -> vector<2x8x16x2xf16>
        %b_val_3 = xegpu.load_nd %B_tile_3 {packed, l1_hint = #xegpu.cache_hint<cached>, l2_hint = #xegpu.cache_hint<cached>, l3_hint = #xegpu.cache_hint<cached>} : !xegpu.tensor_desc<16x16xf16, #xegpu.block_tdesc_attr<array_length =2>> -> vector<2x8x16x2xf16>

        // load A tiles
        %a_val_0 = xegpu.load_nd %A_tile_0 {l1_hint = #xegpu.cache_hint<cached>, l2_hint = #xegpu.cache_hint<cached>, l3_hint = #xegpu.cache_hint<cached>} : !xegpu.tensor_desc<16x16xf16, #xegpu.block_tdesc_attr<array_length =2>> -> vector<2x16x16xf16>
        %a_val_1 = xegpu.load_nd %A_tile_1 {l1_hint = #xegpu.cache_hint<cached>, l2_hint = #xegpu.cache_hint<cached>, l3_hint = #xegpu.cache_hint<cached>} : !xegpu.tensor_desc<16x16xf16, #xegpu.block_tdesc_attr<array_length =2>> -> vector<2x16x16xf16>
        ...
        %next_A_tile_0 = xegpu.update_nd_offset %A_tile_0, [%c0, %c32]  : !xegpu.tensor_desc<16x16xf16, #xegpu.block_tdesc_attr<array_length =2>>
        %next_A_tile_1 = xegpu.update_nd_offset %A_tile_1, [%c0, %c32]  : !xegpu.tensor_desc<16x16xf16, #xegpu.block_tdesc_attr<array_length =2>>

        %next_B_tile_0 = xegpu.update_nd_offset %B_tile_0, [%c32, %c0]  : !xegpu.tensor_desc<16x16xf16, #xegpu.block_tdesc_attr<array_length =2>>
        %next_B_tile_1 = xegpu.update_nd_offset %B_tile_1, [%c32, %c0]  : !xegpu.tensor_desc<16x16xf16, #xegpu.block_tdesc_attr<array_length =2>>
        %next_B_tile_2 = xegpu.update_nd_offset %B_tile_2, [%c32, %c0]  : !xegpu.tensor_desc<16x16xf16, #xegpu.block_tdesc_attr<array_length =2>>
        %next_B_tile_3 = xegpu.update_nd_offset %B_tile_3, [%c32, %c0]  : !xegpu.tensor_desc<16x16xf16, #xegpu.block_tdesc_attr<array_length =2>>

        %b_val_0_flat = vector.shape_cast %b_val_0 : vector<2x8x16x2xf16> to vector<512xf16>
        %b_val_1_flat = vector.shape_cast %b_val_1 : vector<2x8x16x2xf16> to vector<512xf16>
        %b_val_2_flat = vector.shape_cast %b_val_2 : vector<2x8x16x2xf16> to vector<512xf16>
        %b_val_3_flat = vector.shape_cast %b_val_3 : vector<2x8x16x2xf16> to vector<512xf16>
        ...
        // xegpu.compile_hint
        %a_val_0_flat = vector.shape_cast %a_val_0 : vector<2x16x16xf16> to vector<512xf16>
        %a_val_1_flat = vector.shape_cast %a_val_1 : vector<2x16x16xf16> to vector<512xf16>
        ...
        scf.yield %next_A_tile_0, %next_A_tile_1, %next_B_tile_0, %next_B_tile_1, %next_B_tile_2, %next_B_tile_3,
                  ...
                  : !xegpu.tensor_desc<16x16xf16, #xegpu.block_tdesc_attr<array_length =2>>, !xegpu.tensor_desc<16x16xf16, #xegpu.block_tdesc_attr<array_length =2>>,
                  !xegpu.tensor_desc<16x16xf16, #xegpu.block_tdesc_attr<array_length =2>>, !xegpu.tensor_desc<16x16xf16, #xegpu.block_tdesc_attr<array_length =2>>,
                  !xegpu.tensor_desc<16x16xf16, #xegpu.block_tdesc_attr<array_length =2>>,  !xegpu.tensor_desc<16x16xf16, #xegpu.block_tdesc_attr<array_length =2>>,
                  vector<8x16xf32>,vector<8x16xf32>,vector<8x16xf32>,vector<8x16xf32>,vector<8x16xf32>,vector<8x16xf32>,vector<8x16xf32>,vector<8x16xf32>,vector<8x16xf32>,vector<8x16xf32>,vector<8x16xf32>,vector<8x16xf32>,vector<8x16xf32>,vector<8x16xf32>,vector<8x16xf32>,vector<8x16xf32>,
                  !xegpu.tensor_desc<8x32xf16>, !xegpu.tensor_desc<8x32xf16>
      ...
      %c_sg_tile_00 = xegpu.create_nd_tdesc %C[%C_sg_tile_offset_x, %C_sg_tile_offset_y], [%M, %N], [%N, %c1] : memref<?x?xf32> -> !xegpu.tensor_desc<8x16xf32>
    ...
    gpu.func @gemm_kernel_wgX_256_wgY_256_Kblock_32_sgX_32_sgY_64_named_barrier(%A: memref<?x?xf16>, %B: memref<?x?xf16>, %C: memref<?x?xf32>, %M: index, %K: index, %N: index,  %wg_M: index, %wg_K: index, %wg_N: index, %sg_M: index, %sg_K: index, %sg_N: index, %sg_X: index, %sg_Y: index, %prefetch_distance : index, %barrier_distance : index) kernel attributes {VectorComputeFunctionINTEL, spirv.entry_point_abi = #spirv.entry_point_abi<>} {
      // constants
      %c256 = arith.constant 256 : index
      %c512 = arith.constant 512 : index
      %c1024 = arith.constant 1024 : index
      ...
      %num_nbarrier = arith.addi %c8, %c4 : index // 8+4=12
      ...
      %nbarrier_threads_y = arith.index_cast %c8 : index to i8
      ...
      %nbarrier_threads_x = arith.index_cast %c4 : index to i8
      %index_nbarrier_id_x = arith.addi %c4, %local_sg_id_x : index
      ...
      %barrier_nth_iter = arith.muli %c32, %c256 : index
  ...
  func.func @main() attributes {llvm.emit_c_interface} {
    ...
    %wg_K = arith.constant 4096 : index
    ...
    scf.for %i = %c0 to %M step %c1 {
      scf.for %j = %c0 to %N step %c1 {
        memref.store %c0_f32, %C[%i, %j] : memref<?x?xf32>
