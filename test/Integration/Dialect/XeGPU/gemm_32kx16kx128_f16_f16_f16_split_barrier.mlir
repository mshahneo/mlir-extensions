...
module @gemm attributes {gpu.container_module} {
  func.func @test(%A: memref<?x?xf16>, %B: memref<?x?xf16>, %C: memref<?x?xf16>, %M: index, %K: index, %N: index,  %wg_M: index, %wg_K: index, %wg_N: index, %sg_M: index, %sg_K: index, %sg_N: index, %wg_X: index, %wg_Y: index, %wg_Z: index, %sg_X: index, %sg_Y: index, %sg_Z: index, %prefetch_distance: index) -> memref<?x?xf16> attributes {llvm.emit_c_interface} {
    ...
    %C_gpu = gpu.alloc  host_shared (%M, %N) : memref<?x?xf16>
    memref.copy %C, %C_gpu : memref<?x?xf16> to memref<?x?xf16>

    gpu.launch_func  @test_kernel::@test_kernel blocks in (%wg_X, %wg_Y, %wg_Z) threads in (%sg_X, %sg_Y, %sg_Z) args(%A_gpu : memref<?x?xf16>, %B_gpu : memref<?x?xf16>, %C_gpu : memref<?x?xf16>, %M: index, %K: index, %N: index,%wg_M: index, %wg_K: index, %wg_N: index, %sg_M: index, %sg_K: index, %sg_N: index, %sg_X: index, %sg_Y: index, %prefetch_distance : index)

    gpu.dealloc  %A_gpu : memref<?x?xf16>
    gpu.dealloc  %B_gpu : memref<?x?xf16>
    return %C_gpu : memref<?x?xf16>
  }
  gpu.module @test_kernel attributes {spirv.target_env = #spirv.target_env<#spirv.vce<v1.4, [Addresses, Float16Buffer, Int64, Int16, Int8, Kernel, Linkage, Vector16, GenericPointer, Groups, Float16, Float64, AtomicFloat32AddEXT, ExpectAssumeKHR, SubgroupDispatch, VectorComputeINTEL, VectorAnyINTEL], [SPV_EXT_shader_atomic_float_add, SPV_KHR_expect_assume, SPV_INTEL_vector_compute]>, api=OpenCL, #spirv.resource_limits<>>} {
    gpu.func @test_kernel(%A: memref<?x?xf16>, %B: memref<?x?xf16>, %C: memref<?x?xf16>, %M: index, %K: index, %N: index,  %wg_M: index, %wg_K: index, %wg_N: index, %sg_M: index, %sg_K: index, %sg_N: index, %sg_X: index, %sg_Y: index, %prefetch_distance : index) kernel attributes {VectorComputeFunctionINTEL, spirv.entry_point_abi = #spirv.entry_point_abi<>} {
      // constants
  ...
  func.func @main() attributes {llvm.emit_c_interface} {
    ...
    %M = arith.constant 32768 : index
    %K = arith.constant 16384 : index
    %N = arith.constant 128 : index

    // Workgroup tile size for the GPU
    %wg_M = arith.constant 256 : index
    %wg_K = arith.constant 16384 : index
    %wg_N = arith.constant 128 : index

    // Subgroup tile size for the GPU
    %sg_M = arith.constant 32 : index
    %sg_K = arith.constant 32 : index // blocking of K for each iteration
    %sg_N = arith.constant 32 : index


    // Workgroup set up in the gpu
    %wg_X = arith.constant 128 : index
    %wg_Y = arith.constant 1 : index
    ...
    %prefetch_distance = arith.constant 4 : index
    ...
    // run GPU
    %2 = call @test(%A, %B, %C, %M, %K, %N, %wg_M, %wg_K, %wg_N, %sg_M, %sg_K, %sg_N, %wg_X, %wg_Y, %wg_Z, %sg_X, %sg_Y, %sg_Z, %prefetch_distance) : (memref<?x?xf16>, memref<?x?xf16>, memref<?x?xf16>, index, index, index, index, index, index, index, index, index, index, index, index, index, index, index, index) -> memref<?x?xf16>
    ...
    // Debug printout begin
    %nth_row = arith.constant 4096 : index
